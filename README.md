...
- **The CDP proxy is tab-scoped, and idle tabs lose their endpoint.** Each open tab exposes its *own* ephemeral CDP port — so there's no endpoint at all until a tab is open, and Orca **reclaims the debug port of a tab whose renderer has been backgrounded and left idle** (a freshly-created background tab keeps its port; a long-dormant one doesn't). A dormant tab is still *listed* but exposes no endpoint, so attaching to it would otherwise fail. **`attachOrcaTab(pageId)` handles this automatically** — it focuses the tab to wake the renderer and waits for the port to reappear (opt out with `{ activate: false }`). Also note `orca tab list` is **worktree-scoped by default** — the bridge's discovery searches all worktrees, but if you call the CLI directly use `--worktree all`.
