...
- **`attachOrcaTab()` auto-wakes idle tabs.** Orca reclaims the CDP debug port of a tab whose renderer has been backgrounded and left idle, so a pre-existing tab can be *listed* yet expose no endpoint. `attachOrcaTab(pageId)` now focuses such a tab to revive its renderer and waits for the port to reappear, so attaching to a dormant tab "just works". Pass `{ activate: false }` to opt out (clear error instead of stealing focus). New `resolveEndpointForPageId()` export; `findEndpointForPageId()` now searches **all worktrees** (a pageId is global), so cross-worktree attach resolves.
