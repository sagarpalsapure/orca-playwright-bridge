...
| **Idle/backgrounded tab has no CDP endpoint** — Orca reclaims the debug port of a long-dormant tab, so a pre-existing tab is *listed* but not attachable (a fresh background tab keeps its port; an idle one doesn't) | `attachOrcaTab(pageId)` auto-wakes it (focuses the tab, waits for the port). No tab open at all ⇒ no endpoint — open one (`openOrcaTab(url)`). `orca tab list` is worktree-scoped — the bridge searches all worktrees, but use `--worktree all` if you call the CLI yourself |
