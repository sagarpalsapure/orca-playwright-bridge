 ...
 * Start the bridge.
 * @param {object} [opts]
 * @param {RegExp|string} [opts.tab] switch the matching Orca tab active first,
 *   then attach (leverages the Orca CLI — this is how you target a specific tab,
 *   since the CDP proxy only ever exposes the single active one).
 * @param {string} [opts.cdpUrl] override Orca CDP discovery
 * @param {RegExp|string} [opts.match] pick among CDP targets by url. NOTE: Orca's
 *   proxy currently exposes exactly one target (the active tab), so this rarely
 *   matters — use `opts.tab` to choose a tab.
 * @returns {Promise<{url, bridgePort, target, close}>}
 */
async function startBridge(opts = {}) {
  if (opts.tab) await switchToOrcaTab(opts.tab); // make the wanted tab active first
  const WebSocket = loadWs();
  const base = opts.cdpUrl || discoverCdpUrl();
...
module.exports = {
  startBridge, connectOrcaPlaywright, loadChromium, discoverCdpUrl,
