 ...
 */
async function _attachExisting(pageId, { closesTab = false, profileToDelete = null, connectOpts = {}, preferNotIn } = {}) {
  const ep = await findEndpointForPageId(pageId, preferNotIn);
  ...
  conn.reattach = async () => {
    ...
    return _attachExisting(pageId, { closesTab, profileToDelete, connectOpts });
  };
  return conn;
}
...
async function attachOrcaTab(pageId, opts = {}) {
  return _attachExisting(pageId, { closesTab: false, connectOpts: opts });
}
