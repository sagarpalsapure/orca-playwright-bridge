 ...
 */
function findEndpointForPageId(pageId, preferNotIn) {
  const tab = orcaTabList().find((t) => t.browserPageId === pageId);
  if (!tab) throw new Error(`No open Orca tab with pageId ${pageId}.`);
  const eps = discoverAllCdpEndpoints().filter((e) => e.pageUrl === tab.url);
...
async function switchToOrcaTab(match) {
  ...
  if (!tabs[idx].active) {
    ...
    // rather than a fixed sleep — faster when it's quick, safer when it's slow.
    for (let i = 0; i < 30; i++) {
      await sleep(100);
...
async function connectOrcaPlaywright(opts = {}) {
  ...
  // Safe reload: Playwright's page.reload() tears down the tab through Orca's
  // proxy, so re-navigate to the current URL instead (a true document reload
  // that keeps the tab and the bridge alive).
 ...
 * @param {string} url
 * @param {object} [opts] {profile?: string, focus?: boolean}
 *   profile is recorded but Orca does NOT isolate storage today.
 *   focus (default true) brings the new tab to the foreground so you can watch
 *   the run; pass focus:false for background automation.
...
async function openOrcaTab(url, opts = {}) {
  ...
  if (browserPageId && opts.focus !== false) {
    try { execFileSync('orca', ['tab', 'switch', '--page', browserPageId, '--focus', '--json'], { stdio: 'ignore' }); } catch (_) { /* best effort */ }
  }

  // Resolve THIS tab's endpoint by its browserPageId, not by a bare port-diff:
  // if two sessions open tabs at once, a plain diff can grab the other session's
  // freshly-appeared port. Joining on the created pageId's URL (with the diff as
  // a tiebreaker) pins us to the tab we actually created.
  let ep = null;
  for (let i = 0; i < 30 && !ep; i++) {
    await sleep(300);
    if (browserPageId) {
      try { ep = findEndpointForPageId(browserPageId, before); } catch (_) { ep = null; }
    }
    if (!ep) ep = discoverAllCdpEndpoints().find((e) => !before.has(e.port)) || null;
  }
 ...
 * @param {object} [opts] forwarded to connectOrcaPlaywright (connectOptions, …)
 * @returns {Promise<{browser, context, page, bridge, reload, close, browserPageId}>}
 *   close() detaches the bridge but LEAVES the tab open (you didn't create it here).
 */
async function attachOrcaTab(pageId, opts = {}) {
  let ep = null;
  try { ep = findEndpointForPageId(pageId); } catch (e) { throw e; }
  if (!ep) throw new Error(`Tab ${pageId} is open but exposes no CDP endpoint yet.`);
  const conn = await connectOrcaPlaywright({ ...opts, cdpUrl: ep.cdpUrl });
...
module.exports = {
  ...
  discoverAllCdpEndpoints, findCdpUrlForTab, findEndpointForPageId,
