 ...
 *   emulate, clearEmulation, fullPageScreenshot, captureMHTML, axTree, metrics, close }
 */
async function connectOrca(opts = {}) {
  ...
  // --- capture: network -----------------------------------------------------
  /** Start collecting request/response events. Returns { events, stop }. */
  async function recordNetwork() {
    await client.send('Network.enable');
    const events = [];
    const onReq = (e) => events.push({ phase: 'request', id: e.requestId, method: e.request.method, url: e.request.url });
    const onRes = (e) => events.push({ phase: 'response', id: e.requestId, status: e.response.status, mimeType: e.response.mimeType, url: e.response.url });
    const onFail = (e) => events.push({ phase: 'failed', id: e.requestId, error: e.errorText });
    client.on('Network.requestWillBeSent', onReq);
    client.on('Network.responseReceived', onRes);
    client.on('Network.loadingFailed', onFail);
    return { events, stop() { client.off('Network.requestWillBeSent', onReq); client.off('Network.responseReceived', onRes); client.off('Network.loadingFailed', onFail); } };
  }
  ...
  /** Screenshot the FULL page (beyond the viewport). */
  ...
  return {
    ...
    captureConsole, recordNetwork, throttle, offline,
    cookies, setCookie, clearCookies,
    emulate, clearEmulation, fullPageScreenshot, captureMHTML, axTree, metrics,
    close,
  };
}
