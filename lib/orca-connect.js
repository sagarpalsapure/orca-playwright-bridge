...
async function connectOrca(opts = {}) {
  ...
  async function recordNetwork() {
    ...
    const onFin = (e) => { const d = byId.get(e.requestId); if (d) { d.finishTs = e.timestamp; d.size = e.encodedDataLength; } };
      ...
      har() {
        ...
        for (const d of byId.values()) {
          ...
          entries.push({
            ...
            response: d.res
              ? { status: d.res.status, statusText: d.res.statusText || '', httpVersion: d.res.protocol || 'HTTP/1.1', headers: hdrs(d.res.headers), cookies: [], content: { size: d.size || 0, mimeType: d.res.mimeType || '' }, redirectURL: '', headersSize: -1, bodySize: d.size || -1 }
    ...
    return {
      ...
      save(dir) {
        ...
        return frames.map((f, i) => {
          const p = `${dir}/frame-${String(i).padStart(4, '0')}.${ext}`;
  ...
  return {
    ...
    captureConsole, recordNetwork, throttle, offline, blockRequests,
    cookies, setCookie, clearCookies,
    storage, clearStorage,
    emulate, clearEmulation, fullPageScreenshot, captureMHTML, recordScreencast, axTree, domCounters, metrics,
    close,
  };
}
